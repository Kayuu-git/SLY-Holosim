...
(async function() {
    ...
    */
	...
	const craftingProgramPK = new solanaWeb3.PublicKey('CRAFtUSjCW74gQtCS6LyJH33rhhVhdPhZxbPegE4Qwfq');
	const pointsProgramId = new solanaWeb3.PublicKey('PointJfvuHi8DgGsPCy97EaZkQ6NvpghAAVkuquLf3w');
	const pointsStoreProgramId = new solanaWeb3.PublicKey('PsTRqmfqtLcXcGQoguTLAppxq1rnuTqqyUMRoY62XL1');

	const dataRunningXpCategory = new solanaWeb3.PublicKey('DXPsKQPMyaDtunxDWqiKTGWbQga3Wihck8zb8iSLATJQ');
	const councilRankXpCategory = new solanaWeb3.PublicKey('CRXPW3csNpkEYU5U4DUp6Ln6aEEWq4PSUAwV8v6Ygcqg'); //??
	const pilotingXpCategory = new solanaWeb3.PublicKey('PXPfCZwu5Vuuj6aFdEUAXbxudDGeXVktTo6imwhZ5nC');
	const miningXpCategory = new solanaWeb3.PublicKey('MXPkuZz7yXvqdEB8pGtyNknqhxbCzJNQzqixoEiW4Q7');
	const craftingXpCategory = new solanaWeb3.PublicKey('CXPukKpixXCFPrfQmEUGR9VqnDvkUsKfPPLfdd4sKSH8');
	const combatXpAccount = new solanaWeb3.PublicKey('coXptoc2GdykGZpPu4EKHoJXHuWE4GsbkiPiuVH5CB2');
	const LPCategory = new solanaWeb3.PublicKey('LPpdwMuXRuGMz298EMbNcUioaARN8CUU6dA2qyq46g8');
 ...
 */
	...
	async function forceDropFleetCargo(fleet, cargoPod, cargoType, tokenFrom, tokenMint)
	{
		console.log("forceDropFleetCargo");
		console.log(fleet);
		console.log(tokenFrom);
		const [fleetShips] = solanaWeb3.PublicKey.findProgramAddressSync([Buffer.from("FleetShips"), fleet.publicKey.toBuffer()], sageProgramPK);

		let tx1 = {
			instruction: await sageProgram.methods.forceDropFleetCargo().accountsStrict({
				fleet: fleet.publicKey,
		...
		let txResult = await txSignAndSend([tx1], fleet, 'Drop Cargo', 100);
	...
	async function forceDropFleetCargoIfExists(fleet) {
		cLog(1,`${FleetTimeStamp(fleet.label)} Dropping all cargo from all pods if exists...`);
		...
		if (fleetCurrentCargo.value.length > 0) {
			cLog(2,`${FleetTimeStamp(fleet.label)} Found ${fleetCurrentCargo.value.length} item(s) in cargo hold.`);
			for (const tokenAccountInfo of fleetCurrentCargo.value) {
				const mint = tokenAccountInfo.account.data.parsed.info.mint;
				const tokenFrom = tokenAccountInfo.pubkey;
				const cargoType = cargoTypes.find(res => res.account.mint.toString() === mint);
				if (cargoType) {
					await forceDropFleetCargo(fleet, fleet.cargoHold, cargoType, tokenFrom, new solanaWeb3.PublicKey(mint));
				}
			}
		}

		// 2. Handle Fuel Tank
		let fleetCurrentFuel = await solanaReadConnection.getParsedTokenAccountsByOwner(fleet.fuelTank, {programId: tokenProgramPK});
		if (fleetCurrentFuel.value.length > 0) {
			cLog(2,`${FleetTimeStamp(fleet.label)} Found ${fleetCurrentFuel.value.length} item(s) in fuel tank.`);
			for (const tokenAccountInfo of fleetCurrentFuel.value) {
				const mint = tokenAccountInfo.account.data.parsed.info.mint;
				const tokenFrom = tokenAccountInfo.pubkey;
				const cargoType = cargoTypes.find(res => res.account.mint.toString() === mint);
				if (cargoType) {
					await forceDropFleetCargo(fleet, fleet.fuelTank, cargoType, tokenFrom, new solanaWeb3.PublicKey(mint));
				}
			}
		}

		// 3. Handle Ammo Bank
		let fleetCurrentAmmo = await solanaReadConnection.getParsedTokenAccountsByOwner(fleet.ammoBank, {programId: tokenProgramPK});
		if (fleetCurrentAmmo.value.length > 0) {
			cLog(2,`${FleetTimeStamp(fleet.label)} Found ${fleetCurrentAmmo.value.length} item(s) in ammo bank.`);
			for (const tokenAccountInfo of fleetCurrentAmmo.value) {
				const mint = tokenAccountInfo.account.data.parsed.info.mint;
				const tokenFrom = tokenAccountInfo.pubkey;
				const cargoType = cargoTypes.find(res => res.account.mint.toString() === mint);
				if (cargoType) {
					await forceDropFleetCargo(fleet, fleet.ammoBank, cargoType, tokenFrom, new solanaWeb3.PublicKey(mint));
				}
			}
		}
	}
	...
	async function execRespawnToLoadingBay(fleet)
	{
		...
		console.log("Force Drop Cargo");
		await forceDropFleetCargoIfExists(fleet);
		//await forceDropFleetCargoIfExists(fleet, cargoTypes.values());
		console.log("Cargo Droped");
		return new Promise(async resolve => {
			...
			cLog(1,`${FleetTimeStamp(fleet.label)} Respawn`);
			updateFleetState(fleet, 'Respawn')
			console.log("Trigger respawn tx");
	...
	async function execRepairFleet(fleet) {
		return new Promise(async resolve => {
			cLog(1,`${FleetTimeStamp(fleet.label)} Checking fleet for repairs.`);

			// Refresh fleet account info to get current HP
			const fleetAcctInfo = await getAccountInfo(fleet.label, 'full fleet info', fleet.publicKey);
			const fleetData = sageProgram.coder.accounts.decode('fleet', fleetAcctInfo.data);

			const maxHp = fleetData.stats.combatStats.hp;
			const currentHp = fleetData.hp;

			if (currentHp >= maxHp) {
				cLog(1, `${FleetTimeStamp(fleet.label)} Fleet is already at full health.`);
				resolve({ name: "NoRepairsNeeded" });
				return;
			}

			cLog(1, `${FleetTimeStamp(fleet.label)} Fleet requires repairs. HP: ${currentHp}/${maxHp}`);
			updateFleetState(fleet, 'Repairing');

			// Find the starbase the fleet is docked at
			let targetX = fleet.starbaseCoord.split(',')[0].trim();
			let targetY = fleet.starbaseCoord.split(',')[1].trim();
			let starbase = await getStarbaseFromCoords(targetX, targetY);
			let starbasePlayer = await getStarbasePlayer(userProfileAcct, starbase.publicKey);

			// Find a starbase cargo pod that has toolkits
			let starbasePlayerCargoHolds = await cargoProgram.account.cargoPod.all([
				{ memcmp: { offset: 41, bytes: starbasePlayer.publicKey.toBase58() } }
			]);

			let toolkitCargoPod = null;
			let toolkitTokenFrom = null;

			for (const cargoHold of starbasePlayerCargoHolds) {
				if (cargoHold.account && cargoHold.account.openTokenAccounts > 0) {
					let cargoHoldTokens = await solanaReadConnection.getParsedTokenAccountsByOwner(cargoHold.publicKey, { programId: tokenProgramPK });
					let toolkitTokenAccount = cargoHoldTokens.value.find(item => item.account.data.parsed.info.mint === toolItem.token);
					if (toolkitTokenAccount && toolkitTokenAccount.account.data.parsed.info.tokenAmount.uiAmount > 0) {
						toolkitCargoPod = cargoHold.publicKey;
						toolkitTokenFrom = toolkitTokenAccount.pubkey;
						break;
					}
				}
			}
			...
			if (!toolkitCargoPod) {
				cLog(1, `${FleetTimeStamp(fleet.label)} ERROR: No toolkits found at starbase.`);
				updateFleetState(fleet, 'ERROR: No Toolkits');
				resolve({ name: "NoToolkitsFound" });
				return;
			}

			// Get ATLAS token accounts for the repair fee
			const [feeTokenFrom] = await BrowserAnchor.anchor.web3.PublicKey.findProgramAddressSync(
				[ userPublicKey.toBuffer(), tokenProgramPK.toBuffer(), sageGameAcct.account.mints.atlas.toBuffer() ],
				programPK
			);

			const toolkitCargoTypeAcct = cargoTypes.find(item => item.account.mint.toString() == toolItem.token);

			// Build the repair transaction
			let tx = {
				instruction: await sageProgram.methods.repairDockedFleet({
					keyIndex: new BrowserAnchor.anchor.BN(userProfileKeyIdx),
					amount: null // null repairs to full HP
				}).accountsStrict({
					gameAccountsFleetAndOwner: {
						gameFleetAndOwner: {
							fleetAndOwner: {
					...
					cargoPodFrom: toolkitCargoPod,
					cargoType: toolkitCargoTypeAcct.publicKey,
					cargoStatsDefinition: sageGameAcct.account.cargo.statsDefinition,
					tokenFrom: toolkitTokenFrom,
					tokenMint: new solanaWeb3.PublicKey(toolItem.token),
					feeTokenFrom: feeTokenFrom,
					feeTokenTo: sageGameAcct.account.vaults.atlas,
					feeMint: sageGameAcct.account.mints.atlas,
					cargoProgram: cargoProgramPK,
					tokenProgram: tokenProgAddy
				}).remainingAccounts([{
					pubkey: starbase.publicKey,
			...
			cLog(1, `${FleetTimeStamp(fleet.label)} Submitting repair transaction...`);
			let txResult = await txSignAndSend(tx, fleet, 'REPAIR', 100);

			cLog(1, `${FleetTimeStamp(fleet.label)} Repair complete.`);
			updateFleetState(fleet, 'Docked'); // Fleet remains docked after repair
	...
	*/
		...
		if (fleetState == 'Idle' && extra.length > 1 && moveDist && moveX !== null && moveX !== '' && moveY != null && moveY !== '') {
			if (extra[0] !== moveX || extra[1] !== moveY) {
				...
				if ((userFleets[i].moveType == 'warp' || userFleets[i].moveType == 'warp-subwarp-warp' || isStarbaseAndWarpSubwarp) && (currentFuelCnt + currentCargoFuelCnt) >= warpCost && !shortSubwarp && !forceSubwarp) {
					...
					await wait(6000); //Extra wait to ensure accuracy
	...
	async function handleCrewLoading(fleet, starbaseCoords, amount, returnTx) {
		return new Promise(async resolve => {
			...
			console.log(starbasePlayerInfo);
			let availableCrew = starbasePlayerInfo.newTotalCrew - starbasePlayerInfo.busyCrew.toNumber();
	...
	async function operateFleet(i) {
		...
		try {
			...
			{
				await execRespawnToLoadingBay(userFleets[i]);
				await execRepairFleet(userFleets[i]);
			}
			if (fleetState === 'StarbaseLoadingBay' || fleetState === 'Docked') {
				const fleetData = sageProgram.coder.accounts.decode('fleet', fleetAcctInfo.data);
				const maxHp = fleetData.stats.combatStats.hp;
				const currentHp = fleetData.hp;
	...
	function initUser() {
		return new Promise(async resolve => {
			...
			if (globalSettings.saveProfile && globalSettings.savedProfile && globalSettings.savedProfile.length > 0) {
				...
				cLog(2, 'initUser: using saved profile', globalSettings.savedProfile);
				...
				cLog(2, 'initUser: selectedProfile', userProfileAcct.toString());
			...
			console.log("---- INIT ---")
			let fleetCurrentFuelTank = await solanaReadConnection.getParsedTokenAccountsByOwner(userFleets[0].fuelTank, {programId: tokenProgramPK});
			console.log(fleetCurrentFuelTank);
			initComplete = true;
	...
	function waitForLabs(mutations, observer){
		...
		if(document.querySelectorAll(elemTrigger).length > 0 && !document.getElementById("assistContainer")) {
			...
			assistCheckContent.innerHTML = '<div class="assist-modal-header" style="cursor: move;">Fleet Surveillance<div class="assist-modal-header-right"><span class="assist-modal-close">&#x2715;</span></div></div><div class="assist-modal-body"><span id="assist-modal-error"></span><div style="display: flex; flex-direction: row; z-index:20000; justify-content: center;"><select id="fleetGridSelect"><option value="3">3x3</option><option value="5">5x5</option><option value="7">7x7</option></select><input id="checkFleetCntInput" type="text" placeholder="x, y" style="width: 50px;"><button id="checkFleetBtn" class="assist-btn"><span style="font-size: 14px;">Check</span></button></div><div style="display: flex; justify-content: center;"><div id="loadingMessage" style="display: none;">Loading...</div><table id="fleetGrid" class="fleet-grid" style="display: none;"></table></div></div>';
